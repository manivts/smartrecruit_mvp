  ...
  this._seneca.act(args, function(err, entities) {
    ...
    if(err) {
      ...
      for(var i = 0 ; i < entities.length ; i++) {
        ...
        self._deepAuthorize(entityDef, entities[i], args.cmd, args.cmd, roles, context, true, args.showSoftDenied$, function(err, entity) {
          if (!entity) {
            console.log(originalEntity.entity$, originalEntity.id, 'removed in seneca-perm. reason:', err.summary)
          }
          processAuthResultForEntity(err, entity);
        })
      }
    ...
    aclProcedure.authorize(entity, ruleAction, roles, context, function(err, authDecision) {
      if(err) {
        ...
        if(inheritDetails) {
          ...
          // put inheriting entity on context before applying inherited acls
          context.inherit$ = {
            entityDef: entityDef,
            entity: entity
          }
